...
export default function UniversityDetailsPage({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || 'Erreur lors de la mise à jour de l\'université');
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Erreur lors de la suppression de l\'université');
      }
  ...
  if (!university) {
    return <div className="container mx-auto p-6">Université non trouvée</div>;
  }
    ...
    <div className="container mx-auto p-6">
      <div className="mb-6">
        ...
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold">{university.nom}</h1>
